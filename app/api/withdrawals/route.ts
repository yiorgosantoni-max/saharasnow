...
async function balanceForSeller(uid:string){
  ...
  return {profile,available,released,pending};
}

export async function GET(req:NextRequest){
  try{
    const user=await userFromRequest(req);
    const {available}=await balanceForSeller(user.uid);
...
export async function POST(req:NextRequest){
  try{
    ...
    await db.runTransaction(async tx=>{
      ...
      const storedAvailable=Math.max(0,Number(profile?.availableBalanceCents||0));
      const storedReserved=Math.max(0,Number(profile?.pendingWithdrawalCents||0));
      const available=storedReserved>0?storedAvailable:Math.max(0,storedAvailable-pending);
      if(requested>available)throw new Error(`Withdrawal amount cannot exceed your available balance of $${(available/100).toFixed(2)}`);
      ...
      withdrawalId=ref.id;
      tx.create(ref,{sellerId:user.uid,grossCents:gross,feeCents,netCents,status:"pending-manual-payment",currency:CURRENCY,feeRate:WITHDRAWAL_FEE_RATE,requestedAt:new Date()});
      tx.set(userRef,{
        availableBalanceCents:remainingAvailableCents,
    ...
    await notifyUser({userId:user.uid,type:"withdrawal_requested",eventId:withdrawalId,title:"Withdrawal requested",message:`Your withdrawal request for $${(gross/100).toFixed(2)} was submitted.`,link:"/?seller=1",email:true});
    await notifyAdminInApp({type:"withdrawal_requested",eventId:withdrawalId,title:"New withdrawal request",message:`A seller requested a manual withdrawal of $${(gross/100).toFixed(2)}.`,link:"/admin"});
    const resend=new Resend(required("RESEND_API_KEY"));
    const html=`<h1>SaharaSnow withdrawal request</h1><p>Request ${withdrawalId}</p><p>Gross: $${(gross/100).toFixed(2)}</p><p>Fee (5%): $${(feeCents/100).toFixed(2)}</p><p>Manual payout: $${(netCents/100).toFixed(2)}</p>`;
    ...
    return NextResponse.json({id:withdrawalId,currency:CURRENCY,minWithdrawalCents:MIN_WITHDRAWAL_CENTS,grossCents:gross,feeCents,netCents,remainingAvailableCents});
  }catch(e){const x=publicError(e);return NextResponse.json({error:x.message},{status:x.status});}
}
