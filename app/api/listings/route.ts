...
// Every listing title must start with "I will …" (Fiverr-style), enforced here so the
// requirement can't be bypassed by calling the API directly - the form UI locks the
// prefix too, but this is the actual boundary.
const requireIWillTitle = (value: { title: string }) => /^i will\s/i.test(value.title.trim());
const titleRefineOptions = { message: 'Service title must start with "I will".', path: ["title"] };
const schema = listingFields.refine(requireIWillTitle, titleRefineOptions);
const editSchema = listingFields.omit({ category: true, subcategory: true }).refine(requireIWillTitle, titleRefineOptions);
