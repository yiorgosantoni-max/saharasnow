...
const profileSchema = z.object({
  ...
  smsNumber: z.string().trim().min(5).max(20).regex(/^\+?[0-9 ]+$/).optional().or(z.literal(""))
});
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (typeof body.smsNumber === "string" && body.smsNumber.trim()) {
      const existing = (await db.collection("users").doc(user.uid).get()).data() || {};
      if (existing.kycStatus !== "approved") {
        return NextResponse.json({error:"Complete KYC verification before adding an SMS number."},{status:403});
      }
    }
