...
export async function POST(req:NextRequest){try{
 ...
 const stripeMarkupCents=cents(body.amountCents*STRIPE_MARKUP_RATE);
 const platformFeeCents=cents(body.amountCents*PLATFORM_FEE_RATE);
 const checkoutTotalCents=body.amountCents+stripeMarkupCents;
 const invoiceRef=db.collection("customInvoices").doc();
 const appUrl=required("APP_URL").replace(/\/$/,"");
 const session=await stripe.checkout.sessions.create({mode:"payment",payment_method_types:["card"],client_reference_id:user.uid,customer_email:undefined,line_items:[{price_data:{currency:"usd",product_data:{name:body.title,description:body.description||undefined},unit_amount:body.amountCents},quantity:1},{price_data:{currency:"usd",product_data:{name:"Payment processing fee",description:"5% payment processing markup"},unit_amount:stripeMarkupCents},quantity:1}],metadata:{customInvoiceId:invoiceRef.id,buyerId:user.uid,sellerId:body.sellerId,title:body.title,serviceCents:String(body.amountCents),stripeMarkupCents:String(stripeMarkupCents),platformFeeCents:String(platformFeeCents),checkoutTotalCents:String(checkoutTotalCents)},success_url:`${appUrl}/?customInvoiceSuccess={CHECKOUT_SESSION_ID}`,cancel_url:`${appUrl}/?inbox=1&invoiceCanceled=1`});
 await invoiceRef.set({buyerId:user.uid,sellerId:body.sellerId,title:body.title,description:body.description,amountCents:body.amountCents,serviceCents:body.amountCents,stripeMarkupCents,platformFeeCents,checkoutTotalCents,currency:"usd",status:"pending",stripeSessionId:session.id,checkoutUrl:session.url,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()});
 return NextResponse.json({invoiceId:invoiceRef.id,checkoutUrl:session.url,serviceCents:body.amountCents,stripeMarkupCents,platformFeeCents,checkoutTotalCents});
...
export async function PUT(req:NextRequest){try{
 ...
 await db.runTransaction(async tx=>{const latest=await tx.get(invoiceRef); if(latest.data()?.orderId)return; const c=await tx.get(counter); const number=Number(c.data()?.value||1000)+1; tx.set(counter,{value:number},{merge:true}); const serviceCents=Number(invoice.serviceCents??invoice.amountCents);const stripeMarkupCents=Number(invoice.stripeMarkupCents||0);const platformFeeCents=Number(invoice.platformFeeCents||0);const checkoutTotalCents=Number(invoice.checkoutTotalCents??serviceCents+stripeMarkupCents);tx.set(orderRef,{buyerId:user.uid,sellerId:String(invoice.sellerId),listingId:null,orderNumber:number,serviceCents,buyerFeeCents:platformFeeCents,stripeMarkupCents,totalCents:checkoutTotalCents,currency:"usd",title:String(invoice.title),description:String(invoice.description||""),custom:true,status:"in-progress",stripeSessionId:session.id,paidAt:FieldValue.serverTimestamp(),createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()}); tx.set(invoiceRef,{status:"paid",orderId:orderRef.id,paidAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()},{merge:true}); tx.set(orderRef.collection("events").doc(),{type:"order_paid",message:"Custom order payment completed. Work can begin.",actorId:user.uid,createdAt:FieldValue.serverTimestamp()});});
