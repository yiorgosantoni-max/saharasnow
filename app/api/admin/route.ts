...
export async function GET(req: NextRequest) {
  try {
    await admin(req);
    const [kyc, listings, withdrawals, orders, tickets, users, auditLogs, reports, completedOrders] = await Promise.all([
      ...
      db.collection("orders").where("status","==","completed-released").limit(5000).get()
    ]);
    const earningsBySeller = new Map<string, number>();
    const pendingBySeller = new Map<string, number>();
    const availableBySeller = new Map<string, number>();
    completedOrders.docs.forEach(doc => {
      const data = doc.data();
      const sellerId = String(data.sellerId || "");
      if (!sellerId) return;
      earningsBySeller.set(sellerId, (earningsBySeller.get(sellerId) || 0) + Number(data.sellerNetCents ?? data.serviceCents ?? 0));
    });
    orders.forEach(order => {
      const sellerId = String(order.sellerId || "");
      if (!sellerId) return;
      if (["paid","in-progress","delivered"].includes(String(order.status))) {
        pendingBySeller.set(sellerId, (pendingBySeller.get(sellerId) || 0) + Number(order.sellerNetCents ?? order.serviceCents ?? 0));
      }
    });
    users.forEach(user => {
      availableBySeller.set(String(user.id), Number(user.availableBalanceCents || 0));
    });
    const usersWithEarnings = users.map(user => ({
      ...user,
      totalEarningsCents: earningsBySeller.get(String(user.id)) || 0
    }));
    const earnings = users.map(user => ({
      id: String(user.id),
      name: [user.firstName, user.lastName].map(value => String(value || "").trim()).filter(Boolean).join(" ") || String(user.email || user.id),
      email: String(user.email || ""),
      mode: String(user.mode || "BUYING"),
      totalEarningsCents: earningsBySeller.get(String(user.id)) || 0,
      pendingEarningsCents: pendingBySeller.get(String(user.id)) || 0,
      availableBalanceCents: availableBySeller.get(String(user.id)) || 0
    })).sort((a,b) => b.totalEarningsCents - a.totalEarningsCents);
    return NextResponse.json({adminEmail: ADMIN_EMAIL, kyc, listings, withdrawals, orders, tickets, users: usersWithEarnings, earnings, auditLogs, reports});
  } catch (error) {
