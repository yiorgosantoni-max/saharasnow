  ...
  try{
    ...
    for(const d of snap.docs){
      ...
      if(!o.clearanceEndsAt){
        ...
        // Start the standard three-day clearance countdown now.
        ...
        await db.runTransaction(async tx=>{
          ...
          tx.set(d.ref,{clearanceStartsAt:FieldValue.serverTimestamp(),clearanceEndsAt:new Date(now+3*24*60*60*1000),updatedAt:FieldValue.serverTimestamp()},{merge:true});
        });
        if(sellerId){
          advanced++;
          const number=String(o.orderNumber||d.id);
          await notifyUser({userId:sellerId,type:"order_review_window_passed",eventId:`${d.id}_clearance_started`,title:`Order #${number} entered clearance`,message:"The buyer review window closed with no dispute. Your earnings are now in the standard three-day clearance period.",link:"/?seller=1",email:true});
          if(buyerId)await notifyUser({userId:buyerId,type:"order_review_window_passed",eventId:`${d.id}_clearance_started_buyer`,title:`Order #${number} entered clearance`,message:"Your review window has closed. The order is now in the standard three-day clearance period before the seller is paid.",link:`/?order=${d.id}`});
        }
        continue;
      }

      // Stage 2 -> release: the three-day clearance has ended. Release the seller's full
      ...
      await db.runTransaction(async tx=>{
        ...
        tx.set(ledgerRef,{userId:sellerId,type:"clearance-release",amountCents:netCents,currency,orderId:d.id,status:"released",createdAt:FieldValue.serverTimestamp(),releasedBy:"automatic-three-day-clearance"});
      ...
      if(sellerId&&netCents>0){
        ...
        await notifyUser({userId:sellerId,type:"earnings_released",eventId:`${d.id}_automatic_release`,title:"Three-day clearance completed",message:`Your ${netAmount} ${currency} was automatically released to your available balance. The 5% fee is only deducted when you request a withdrawal.`,link:"/?seller=1",email:true});
        released++;
      }
    }
