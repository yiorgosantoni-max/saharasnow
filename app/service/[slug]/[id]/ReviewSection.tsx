...
export default function ReviewSection({sellerId}:{sellerId:string}){
 ...
 return <section style={{marginTop:32,paddingTop:28,borderTop:"1px solid #e7e2da"}}>
  ...
  {loading?<p>Loading reviews…</p>:reviews.length===0?<p style={{color:"#777"}}>No reviews yet. Completed buyers can leave the first verified review.</p>:<div style={{display:"grid",gap:14,marginTop:18}}>
   {reviews.map(r=><article key={r.id} style={{padding:"16px 18px",border:"1px solid #e7e2da",borderRadius:14}}>
    ...
    {r.sellerReply&&<div style={{marginTop:14,marginLeft:18,paddingLeft:14,borderLeft:"3px solid #d9d3ea",background:"#faf9fd",borderRadius:"0 10px 10px 0",padding:"12px 14px"}}>
    ...
    {isSeller&&(replyOpen===r.id?<div style={{marginTop:12,marginLeft:18}}>
     ...
     <div style={{display:"flex",gap:8,marginTop:8}}>
      <button type="button" onClick={()=>{setReplyOpen("");setReplyText("");setReplyError("")}} style={{padding:"9px 14px",borderRadius:9,border:"1px solid #ccc",background:"#fff",cursor:"pointer"}}>Cancel</button>
      <button type="button" disabled={replyBusy} onClick={()=>void submitReply(r.id)} style={{padding:"9px 14px",borderRadius:9,border:0,background:"#17134f",color:"#fff",fontWeight:700,cursor:"pointer"}}>{replyBusy?"Posting…":"Post reply"}</button>
     </div>
    </div>:<button type="button" onClick={()=>{setReplyOpen(r.id);setReplyText(r.sellerReply||"");setReplyError("")}} style={{marginTop:12,marginLeft:18,padding:"8px 13px",borderRadius:9,border:"1px solid #d9d3ea",background:"#fff",fontWeight:700,fontSize:13,cursor:"pointer"}}>{r.sellerReply?"✎ Edit your reply":"💬 Reply to this review"}</button>)}
   </article>)}
  </div>}
 </section>;
}
