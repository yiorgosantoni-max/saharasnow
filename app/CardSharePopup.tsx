...
type ShareKind = "facebook" | "x" | "whatsapp" | "copy";
...
export default function CardSharePopup() {
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .sahara-detail-share-row{display:flex;align-items:center;gap:10px;margin-top:18px;padding-top:14px;border-top:1px solid #eaecf0}
      .sahara-detail-share-count{font-size:13px;color:#667085}
      .sahara-detail-share-button{margin-left:auto;border:1px solid #d0d5dd;background:#fff;border-radius:10px;width:40px;height:40px;cursor:pointer;color:#344054;font-size:20px}
    ...
    const addDetailShare = () => {
      if (!/^\/service\/[^/]+\/[^/]+\/?$/.test(window.location.pathname)) return;
      const article = document.querySelector<HTMLElement>("main article");
      if (!article || article.querySelector("[data-sahara-detail-share]") || !article.querySelector("h1")) return;
      const id = window.location.pathname.split("/").filter(Boolean).pop() || "";
      if (!id) return;
      const row = document.createElement("div");
      row.className = "sahara-detail-share-row";
      row.dataset.saharaDetailShare = "true";
      row.innerHTML = `<span class="sahara-detail-share-count" data-sahara-share-count-id="${id}">0 shares · 0 favourites</span><button type="button" class="sahara-detail-share-button" data-sahara-share data-listing-id="${id}" aria-label="Share this service" title="Share this service">↗</button>`;
    ...
    const openFor = (trigger: Element) => {
      ...
      activeUrl = suppliedUrl || serviceLink?.href || window.location.href;
      if (!activeId) return;
    ...
    addDetailShare();
    const observer = new MutationObserver(addDetailShare);
