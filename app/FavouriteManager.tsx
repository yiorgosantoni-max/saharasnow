...
export default function FavouriteManager(){
 ...
 useEffect(()=>{
   ...
   const attach=()=>{
     const root=document.querySelector<HTMLElement>(".accountActions");if(!root||current&&root.contains(current))return;
     current=root.querySelector<HTMLButtonElement>("[data-favourites-trigger]");
     if(!current){current=document.createElement("button");current.type="button";current.setAttribute("data-favourites-trigger","true");current.setAttribute("aria-label","Favourites");current.textContent="♡";root.insertBefore(current,root.firstChild);}
     current.onclick=()=>{if(!signedInRef.current){setNotice("Please sign in to save services.");return}setOpen(true);void load()};
   };
   attach();const observer=new MutationObserver(attach);observer.observe(document.body,{childList:true,subtree:true});return()=>{observer.disconnect();if(current?.parentElement)current.remove()};
 },[]);
