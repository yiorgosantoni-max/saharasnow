...
import {countryFlag,resolveCountryCode} from "@/lib/country";
import styles from "./seller.module.css";

type Params={slug:string;id:string};
type PublicSeller={id:string;name:string;country:string;countryCode:string;image:string;joinedAt:Date|null;kycVerified:boolean;services:Array<Record<string,unknown>>};
...
async function getSeller(id:string):Promise<PublicSeller|null>{
  ...
  return {id,name,country,countryCode,image:String(profile.profileImageUrl||""),joinedAt:asDate(profile.joinedAt),kycVerified:profile.kycStatus==="approved",services};
}
...
export default async function SellerPage({params}:{params:Promise<Params>}){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.profile}>
      ...
      <div><p className={styles.eyebrow}>SELLER PROFILE</p><h1>{seller.name}</h1><p className={styles.meta}><span aria-hidden="true">{countryFlag(seller.countryCode)}</span> {seller.country||"Country not provided"}{seller.joinedAt?` · Joined ${seller.joinedAt.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}`:""}</p>{seller.kycVerified&&<span className={styles.verified}>✓ KYC verified</span>}</div>
    </section>
