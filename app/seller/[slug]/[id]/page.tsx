...
type PublicSeller={id:string;name:string;country:string;image:string;joinedAt:Date|null;kycVerified:boolean;services:Array<Record<string,unknown>>;totalEarningsCents:number};
...
async function getSeller(id:string):Promise<PublicSeller|null>{
  ...
  const [listingSnap,orderSnap]=await Promise.all([db.collection("listings").where("sellerId","==",id).limit(100).get(),db.collection("orders").where("sellerId","==",id).limit(500).get()]);
  const services=listingSnap.docs.filter(doc=>doc.data().status==="active").map(doc=>({id:doc.id,...doc.data()}));
  const totalEarningsCents=orderSnap.docs.filter(doc=>doc.data().status==="completed-released").reduce((sum,doc)=>sum+Number(doc.data().sellerNetCents??doc.data().serviceCents??0),0);
  return {id,name,country,image:String(profile.profileImageUrl||""),joinedAt:asDate(profile.joinedAt),kycVerified:profile.kycStatus==="approved",services,totalEarningsCents};
}
...
export default async function SellerPage({params}:{params:Promise<Params>}){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.profile}>
      ...
      <div><p className={styles.eyebrow}>SELLER PROFILE</p><h1>{seller.name}</h1><p className={styles.meta}>{seller.country||"Country not provided"}{seller.joinedAt?` · Joined ${seller.joinedAt.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}`:""}</p>{seller.kycVerified&&<span className={styles.verified}>✓ KYC verified</span>}<div className={styles.earnings}><small>Total earnings</small><strong>€{(seller.totalEarningsCents/100).toFixed(2)}</strong><span>Released and paid orders</span></div></div>
    </section>
