...
import { Auth, browserLocalPersistence, getAuth, indexedDBLocalPersistence, initializeAuth, onAuthStateChanged, signInWithCustomToken, signOut } from "firebase/auth";
...
export const getClientAuth=()=>{if(cachedAuth)return cachedAuth;const app=clientApp();try{cachedAuth=initializeAuth(app,{persistence:[indexedDBLocalPersistence,browserLocalPersistence]})}catch{cachedAuth=getAuth(app)}return cachedAuth;};
