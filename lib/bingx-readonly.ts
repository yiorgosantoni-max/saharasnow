...
export async function bingxGetDeposits(coin: "USDT" | "USDC", startTime: number, endTime: number): Promise<BingXDeposit[]> {
  ...
  const url = `${BASE_URL}/openApi/api/v3/capital/deposit/hisrec?${query}&signature=${signature}`;
  const response = await fetch(url, { headers: { "X-BX-APIKEY": API_KEY!, "X-SOURCE-KEY": "SAHARASNOW" }, cache: "no-store" });
...
export async function findBingXDepositByHash(txHash: string, coins: Array<"USDT" | "USDC"> = ["USDT", "USDC"]) {
  ...
  for (const coin of coins) {
    ...
    const match = deposits.find((d) => String(d.txId || "").trim().toLowerCase() === normalized);
